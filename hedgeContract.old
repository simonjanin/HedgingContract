...
function bytes32ToString (bytes32 data) returns (string) {
    ...
    return string(bytesString);
      ...
      function getPriceAtomic() returns uint {
          bytes memory to = new bytes(10);
          bytes memory from_bytes = bytes(bytes32ToString(uintToBytes(block.timestamp)));
          
          return parseInt(oraclize_query("URL", strConcat("json(https://www.cryptocompare.com/api/data/pricehistorical?fsym=ETH&tsyms=USD&ts=", string(copyBytes(from_bytes, 0, 10, to, 0)), ").Data.0.Price")), 6);
      }
        ...
        function refund(address hedger) {
                ...
                if (offerers[hedger].blockTimestamp + offerers[hedger].maturity <= block.timestamp) { // maturity has been reached
                        if (PT >= P0) { //w  w) { // PT >= P0
                                Print("Bande de putain de sales putes");
