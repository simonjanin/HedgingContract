...
contract HedgerContract is usingOraclize {
	...
	function update() {
        ...
        Print("Bande de putains");		//Myevent(strConcat("json(https://www.cryptocompare.com/api/data/pricehistorical?fsym=ETH&tsyms=USD&ts=", string(copyBytes(from_bytes, 0, 10, to, 0)), ").Data.0.Price"));
	}
	...
	function refund(address hedger) {
		...
		if (endTimestamp <= block.timestamp) { // maturity has been reached
			...
			uint PT = offerers[hedger].endingPrice; // TODO: query Oraclize to get price at timestamp offerers[hedger].blockTimestamp (speculator's acceptation of offer)
